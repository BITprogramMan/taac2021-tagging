...
#################Inference Utils#################
tokenizer = tokenization.FullTokenizer(vocab_file='pretrain_models/robert/chinese_L-12_H-768_A-12/vocab.txt')
class TaggingModel():
...
#                              extract_video = True, extract_audio = True, extract_text = False)
    ...
    def text_preprocess(self, txt,text_type='ocr',max_len=300):
        
        if text_type=='asr':
            txt_ocr=txt['video_asr']
        else:
            txt_ocr=txt['video_ocr']
        
        tokens = ['[CLS]'] + tokenizer.tokenize(txt_ocr)
        ids = tokenizer.convert_tokens_to_ids(tokens)
        if len(ids)>max_len:
            str_list_ocr=list(txt['video_ocr'])
            str_list_asr=list(txt['video_asr'])
            common_text = [val for val in str_list_asr if val in str_list_ocr]
            if len(common_text)>100 :
                text=''.join(common_text)
    ...
    def preprocess(self, feat_dict, max_frame_num=62,text_type='ocr'):
        ret_dict = {}
        for feat_type in feat_dict:
            if feat_type=='video':
                ...
                feats[:valid_num] = feat_dict['video'][:valid_num]
            elif feat_type=='audio':
                feats = np.zeros((max_frame_num, len(feat_dict['audio'][0])))
                valid_num = min(max_frame_num, len(feat_dict['audio']))
                feats[:valid_num] = feat_dict['audio'][:valid_num]
            elif feat_type=='text':
                feats = self.text_preprocess(feat_dict['text'],text_type = text_type, max_len=300)
    ...
    def load_multimodal_feat(self, test_file, feat_dir):
        ...
        with open(os.path.join(feat_dir, 'text_txt', 'tagging', video_id + '.txt'),'r') as f:
            text = json.load(f)
        feat_dict['text'] = text

#         feat_dict['text'] = open().read()
        return feat_dict

    def inference(self, test_file, load_feat=False, feat_dir=None,text_type='ocr'):
...
#                 print(feat_dict['text'])
                ...
                print('error!!!!!!!!!!!!!')
                feat_dict['text'] = ""
            feat_dict_preprocess = self.preprocess(feat_dict, max_frame_num=62,text_type=text_type)
            feed_dict ={}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--change_model_pb', action="store_false")
    parser.add_argument('--text_type', type=str, default='ocr')
    ...
    if args.change_model_pb:
        base_root = args.model_pb
        file = os.listdir(base_root)
        args.model_pb = base_root + file[0]
...
#     base_root='/home/tione/notebook/algo-2021/dataset/videos/test_5k_2nd/'
#     base_root='/home/tione/notebook/algo-2021/dataset/videos/video_5k/train_5k/'
#     test_files=os.listdir(base_root)
    
    
################################################################################

    base_root=os.path.dirname(args.output)
    ...
    for test_file in test_files:
        print(test_file)
        try:
          labels, scores = model.inference(test_file, args.load_feat, args.feat_dir,text_type=args.text_type)
        except:
        ...
        if args.output_json is not None:
            ...
            cur_output["result"] = [{"labels": labels[:args.top_k], "scores": ["%.5f" % scores[i] for i in range(args.top_k)]}]
#             print(cur_output)
...
#                 f.write("{}\t{}\n".format(video_id, "\t".join(["{}##{:.5f}".format(labels[i], scores[i]) for i in range(len(scores))])))
        print("-"*100)
