...
if __name__ == '__main__':
    ...
    parser.add_argument('--result6', default=None, type=str)
    parser.add_argument('--result7', default=None, type=str)
    parser.add_argument('--result8', default=None, type=str)
    ...
    assert args.result6 is not None
    assert args.result7 is not None
    assert args.result8 is not None
    ...
    with open(args.result5) as f:
        part5 = json.load(f)
    with open(args.result6) as f:
        part6 = json.load(f)
    with open(args.result7) as f:
        part7 = json.load(f)
    with open(args.result8) as f:
        part8 = json.load(f)
    ...
    for file in part6.keys():
        label_set = set(part6[file]['result'][0]['labels'])
        res_label = list(all_label_set - label_set)
        part6[file]['result'][0]['labels'] += res_label
        part6[file]['result'][0]['scores'] += ['0'] * 32
        assert len(res_label) == 32
        assert len(part6[file]['result'][0]['labels']) == 82
    for file in part7.keys():
        label_set = set(part7[file]['result'][0]['labels'])
        res_label = list(all_label_set - label_set)
        part7[file]['result'][0]['labels'] += res_label
        part7[file]['result'][0]['scores'] += ['0'] * 32
        assert len(res_label) == 32
        assert len(part7[file]['result'][0]['labels']) == 82
    for file in part8.keys():
        label_set = set(part8[file]['result'][0]['labels'])
        res_label = list(all_label_set - label_set)
        part8[file]['result'][0]['labels'] += res_label
        part8[file]['result'][0]['scores'] += ['0'] * 32
        assert len(res_label) == 32
        assert len(part8[file]['result'][0]['labels']) == 82
    w1 = 0.125
    w2 = 0.125
    w3 = 0.125
    w4 = 0.125
    ...
    for file in part1.keys():
        ...
        for index, label in enumerate(part1[file]['result'][0]['labels']):
            ...
            index6 = part6[file]['result'][0]['labels'].index(label)
            index7 = part7[file]['result'][0]['labels'].index(label)
            index8 = part8[file]['result'][0]['labels'].index(label)
            ...
            score = (w1 * eval(part1[file]['result'][0]['scores'][index1]) +
                     ...
                     w6 * eval(part6[file]['result'][0]['scores'][index6]) +
                     w7 * eval(part7[file]['result'][0]['scores'][index7]) +
                     w8 * eval(part8[file]['result'][0]['scores'][index8]) )
